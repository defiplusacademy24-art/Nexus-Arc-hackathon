...
| Members / join | cooperative routes |
| Transactions | `POST/GET /api/transactions` |
| Notifications | `GET /api/notifications` |
| On-chain helpers | `/api/onchain/*` |
| Agents | `GET /api/agents/health`, audit, events, Nexa ask |

Autonomous agents run on a **long-lived worker** (`AGENTS_ENABLED=true`), not serverless alone.

See also: [Agent & Circle Wallet Setup](./AGENT_AND_CIRCLE_WALLET_SETUP.md)

---

## Database

Cooperatives, members, contributions/transactions, loans (app ledger + on-chain), notifications, governance, agent events/tasks/memory/audit, platform data.
