  ...
  return (
    <button
      ...
      className={cn(
        ...
        enabled ? 'bg-[#E8461E]' : 'bg-stone-200 dark:bg-white/15',
      )}
    >
      <span className={cn(
...
const SETTINGS: SettingsSection[] = [
  ...
  },
  {
    section: 'Privacy',
    rows: [
      { label: 'Show wallet address to members', description: 'Display your Unicity wallet address in the member directory', type: 'toggle', defaultOn: false },
      ...
      <div className="px-6 py-6 max-w-2xl mx-auto">
        ...
        >
          <div className="flex items-center gap-2 mb-4">
            <Palette className="w-4 h-4 text-[#E8461E]" />
              ...
              <button
                ...
                className={cn(
                  ...
                  resolvedTheme === key || (key === 'system' && !['light', 'dark'].includes(resolvedTheme ?? ''))
                    ? 'border-[#E8461E] bg-[#E8461E]/5 dark:bg-[#E8461E]/10 text-[#E8461E]'
            ...
            >
              ...
              {section.rows.map((row) => (
                <SettingRow key={row.label} label={row.label} description={row.description}>
                  ...
                  {row.type === 'select' && (
                    <select className="text-xs bg-stone-50 dark:bg-white/5 border border-stone-200 dark:border-white/10 rounded-lg px-2.5 py-1.5 text-stone-700 dark:text-white/70 outline-none cursor-pointer flex-shrink-0">
